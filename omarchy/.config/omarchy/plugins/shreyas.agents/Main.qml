...
// The display side of agent usage. All extraction lives behind the local
// usage updater, which writes one JSON record per agent into
...
Item {
  ...
  function updateCommand(kind, agentIds) {
    // Keep the provider collectors user-owned. The packaged updater currently
    // invokes an obsolete Codex CLI approval value, and this clone also adds
    // the Grok collector without touching /usr/share/omarchy.
    var user = Quickshell.env("USER") || "user"
    var command = [root.home + "/.config/omarchy/plugins/" + user + ".agents/usage-update"]
