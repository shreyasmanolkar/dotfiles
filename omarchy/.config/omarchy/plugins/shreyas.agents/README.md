...
The panel is strictly a display: it watches the usage records that the local
`usage-update` wrapper writes to `~/.local/state/omarchy/agents/usage/`
...
| `codex` | The Codex app-server RPC | native Codex CLI session files (plus pi and opencode sessions); uses the current `--ask-for-approval never` spelling |
| `grok` | Grok CLI's locally cached weekly billing status | Grok's local `unified.jsonl` inference records, grouped by day and model |
